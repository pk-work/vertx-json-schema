...
public class JsonObjectProxy extends JsonObject {
  ...
  public String encode() {
    ...
    try {
      return encode(sw, generator, false);
    } catch (StackOverflowError err) {
      // Schema contains a circular reference
      sw = new StringWriter();
      generator = createGenerator(sw, false);
      return encode(sw, generator, true);
    }
  }
  ...
  public String encodePrettily() {
    ...
    try {
      ...
      // Schema contains a circular reference
      sw = new StringWriter();
      generator = createGenerator(sw, true);
      return encode(sw, generator, true);
    }
  }

  private String encode(StringWriter sw, JsonGenerator generator, boolean replaceRefs) {
    encodeJson(this, generator, replaceRefs);
  ...
  private static void encodeJson(Object json, JsonGenerator generator, boolean replaceRefs) throws EncodeException {
    try {
      if (replaceRefs && json instanceof JsonObjectRef) {
      ...
      if (json instanceof Map) {
        ...
        for (Map.Entry<String, ?> e : ((Map<String, ?>) json).entrySet()) {
          ...
          encodeJson(e.getValue(), generator, replaceRefs);
        }
        generator.writeEndObject();
      } else if (json instanceof List) {
        generator.writeStartArray();
        for (Object item : (List<?>) json) {
          encodeJson(item, generator, replaceRefs);
        }
